...
- ~AOF~ polega na dopisywaniu każdego polecenia wykonywanego na bazie do pliku append-only.
...
- W przyszłości planowane jest połączenie ~RDB~ i ~AOF~ w jeden wspólny mechanizm.
...
- Chyba że np. fork się nie udał bo nie mamy wystarczająco dużo wolnego RAMU, wtedy nie mamy wyboru.

* BGSAVE
- ~BGSAVE~ działa asynchronicznie, forkuje proces potomny który wykonuje zrzut.
- Fork może się nie udać bo nie mamy wystarczająco dużo wolnego RAMU.
